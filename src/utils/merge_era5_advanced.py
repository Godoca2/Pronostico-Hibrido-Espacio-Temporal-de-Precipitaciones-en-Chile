...
def merge_era5_files(variable: str = "total_precipitation") -> xr.Dataset:
    ...
    if not files:
        raise FileNotFoundError("❌ No se encontraron archivos NetCDF en data/raw/.")
    print(f"📦 {len(files)} archivos encontrados. Combinando con xarray...")

    ds = xr.open_mfdataset(files, combine="by_coords", parallel=True, chunks={"time": 500})

    if variable not in ds.data_vars:
        variable = list(ds.data_vars)[0]
        print(f"⚠️ Variable '{variable}' seleccionada automáticamente.")
    ds = ds[[variable]]
    ds = ds.sortby(["time", "latitude", "longitude"])

    out_nc = OUT_DIR / "era5_precipitation_chile_full.nc"
    ds.to_netcdf(out_nc)
    print(f"✅ Dataset combinado guardado: {out_nc}")
    return ds
...
def compute_daily_means(ds: xr.Dataset, variable: str = "total_precipitation") -> pd.DataFrame:
    ...
    print(f"✅ Datos diarios exportados: {out_csv}")
    return df_daily

...
def download_chirps_daily(
    ...
    print(f"📥 Descargando datos CHIRPS para Chile")
    ...
    if datasets:
        print(f"\n🔗 Concatenando {len(datasets)} dataset(s)...")
        ds_combined = xr.concat(datasets, dim='time')
        
        # Guardar dataset combinado
        output_combined = output_path / f"chirps_chile_{start_date}_{end_date}.nc"
        print(f"💾 Guardando dataset combinado: {output_combined}")
...
def compare_with_era5(
    ...
    print(f"📊 Comparando CHIRPS vs ERA5...")
    ...
    print(f"\n⚠️ Comparación detallada pendiente de implementación")
    print(f"   Sugerencias:")

...
from models.ae_keras import build_autoencoder, build_autoencoder_lstm, train_autoencoder, get_encoder, encode, reconstruct
...
class AE_DMD:
    ...
    def __init__(self, latent_dim: int = 32, use_lstm: bool = False, dmd_rank: int = None):
        """
        Parámetros
        ----------
        latent_dim : int
        ...
        self.dmd_rank = dmd_rank if dmd_rank is not None else min(latent_dim, 20)
        self.autoencoder = None
        self.encoder = None
        self.dmd = None
        self.training_history = None

    def fit_autoencoder(self, X_train: np.ndarray, epochs: int = 50, batch_size: int = 32, 
    ...
    def fit_dmd(self, X_time_series: np.ndarray):
        ...
        if self.encoder is None:
            raise ValueError("Debes entrenar el autoencoder primero con fit_autoencoder()")
        
        print(f"[AE-DMD] Codificando {X_time_series.shape[0]} snapshots al espacio latente...")
        Z = encode(self.autoencoder, X_time_series)
        
        # DMD espera snapshots en columnas: (latent_dim, T)
        snapshots = Z.T
        print(f"[AE-DMD] Aplicando DMD sobre snapshots latentes: shape={snapshots.shape}")
        
        self.dmd = DMD(svd_rank=self.dmd_rank)
        self.dmd.fit(snapshots)
        
        print(f"[AE-DMD] DMD ajustado: {len(self.dmd.modes.T)} modos, rango={self.dmd_rank}")
        return self
    ...
    def forecast(self, steps: int = 10, method: str = 'dmd') -> np.ndarray:
        ...
        if method == 'dmd':
            ...
            future_times = np.arange(last_time_idx + 1, last_time_idx + steps + 1)
        ...
        print(f"[AE-DMD] Decodificando {Z_future.shape[0]} estados latentes...")

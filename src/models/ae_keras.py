...
def build_autoencoder_lstm(timesteps: int, n_features: int, latent_dim: int = 64) -> models.Model:
    ...
    encoded = layers.LSTM(128, activation='relu', return_sequences=True)(input_layer)
...
def train_autoencoder(X_train: np.ndarray, latent_dim: int = 32, epochs: int = 50, 
                     batch_size: int = 32, use_lstm: bool = False, validation_split: float = 0.15):
    ...
    if use_lstm:
        if len(X_train.shape) != 3:
            raise ValueError("Para LSTM, X_train debe tener shape (samples, timesteps, features)")
        ...
        if len(X_train.shape) != 2:
            raise ValueError("Para AE denso, X_train debe tener shape (samples, features)")
...
def encode(model: models.Model, X: np.ndarray, batch_size: int = 32) -> np.ndarray:
    """Retorna representaciones latentes del autoencoder."""
    encoder = get_encoder(model)
    return encoder.predict(X, batch_size=batch_size, verbose=0)


def reconstruct(model: models.Model, X: np.ndarray, batch_size: int = 32) -> np.ndarray:
